    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $row['password'])) {
            ...
            if ($row['is_admin'] == 1) {
                ...
                // Non-admin user, redirect to department page
                $department = strtolower($row['department']);
                switch ($department) {
                    case 'facilities management':
                        $departmentPage = 'facilities_management.html';
                        break;
                    case 'operations':
                        $departmentPage = 'operations_department.html';
                        break;
                    case 'training':
                        $departmentPage = 'training_department.html';
                        break;
                    case 'supply':
                        $departmentPage = 'supply_department.html';
                        break;
                    default:
                        // Redirect to a default page if department not recognized
                        $departmentPage = 'index.html';
                        break;
                }
                header("Location: $departmentPage");
                exit();
            }
        } else {
            // Incorrect password
