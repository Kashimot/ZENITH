...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['requestId'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($result_admin->num_rows > 0) {
            ...
            $sql_insert = "INSERT INTO facility_history (date, facilityName, granted_by) VALUES (NOW(), ?, ?)";
            $stmt_insert = $conn->prepare($sql_insert);
            $stmt_insert->bind_param("ss", $facilityName, $grantedBy);

            // Execute the SQL statement
            if ($stmt_insert->execute()) {
                // Update status in pending_requests table to granted
                $sql_update = "UPDATE pending_requests SET status = 'granted' WHERE id = ?";
                $stmt_update = $conn->prepare($sql_update);
                $stmt_update->bind_param("i", $requestId);
                $stmt_update->execute();
                echo "Facility request granted successfully";
            } else {
                echo "Error granting facility request: " . $stmt_insert->error;
            }
